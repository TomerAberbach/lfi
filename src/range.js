...
export const rangeUntilAsync = curry((start, end) => {
  async function* iterate(step) {
    if (start < end) {
      for (let i = start; i < end; i += step) {
        yield i
      }
...
export const rangeToAsync = curry((start, end) =>
  rangeUntilAsync(start, end + (start < end ? 1 : -1))
)
